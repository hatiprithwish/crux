...
const ZTrackerFormValues = z
  ...
  .superRefine((values, ctx) => {
    ...
    if (values.metricMode === "new" && values.metricName.trim() === "") {
      ctx.addIssue({ code: "custom", path: ["metricName"], message: "Metric name is required" });
    }
    ...
    >
      ...
      <div className="flex gap-4">
        <form.Field name="target">
          {(field) => (
            <Field>
              <FieldLabel htmlFor={field.name}>Target (optional)</FieldLabel>
              <Input
                id={field.name}
                type="number"
                step="any"
